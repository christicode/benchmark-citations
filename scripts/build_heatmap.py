...
.chip.select-all{font-family:inherit;line-height:inherit}
...
thead th{position:relative;border-bottom:1px solid var(--b)}
th.mh{height:130px;min-width:28px;vertical-align:bottom;transition:background-color 0.15s}
...
<div class=bar>
  ...
  <div class=grp><b>company</b>
    <div class=seg id=cos-seg><button type=button class="chip select-all" id=select-all-cos onclick="setAllCos(true)">all</button></div>
  </div>
</div>
...
function render(){
  var allSelected=DATA.companies.every(function(c){return state.cos[c.id];});
  ...
  more.textContent=state.multi?'Expand to see more':'Show only 2+ citations';
