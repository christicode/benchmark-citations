...
agg = collections.defaultdict(lambda: {"raws": set(), "axis": "agentic", "hs": "not_in_harbor", "hn": None})
for line in open(ROOT / "data" / "citations.jsonl"):
    ...
    if not canon:
        continue
    a = agg[canon]
    a["raws"].add(c["benchmark_raw"]); a["axis"] = c.get("axis", "agentic")
...
out = ["# aliases_generated.yaml - AUTO-GENERATED from citations.jsonl. DO NOT EDIT.",
       "# Curated core + collision flags live in aliases.yaml.", "schema_version: 1", "benchmarks:"]
for c, v in sorted(agg.items()):
    out.append(f"  - canonical: {c}")
    out.append(f"    axis: {v['axis']}")
