...
Reads data/citations.jsonl (each record self-describes on_harbor / harbor_status / axis).
Live-fetches Harbor registry.json from main only to report size + sanity-check dedupe.

Score (agentic/static kept SEPARATE, surfaced not folded in):
...
PW = {"headline": 3.0, "table_row": 1.0, "footnote": 0.5, "prose": 0.5}
...
def main() -> None:
    ...
    agg = collections.defaultdict(lambda: {"usage": 0.0, "labs": set(), "docs": 0,
                                           "solve": [], "axis": "?", "status": "not_in_harbor",
    ...
    for line in open(ROOT / "data" / "citations.jsonl"):
        ...
        if not canon:
            continue
        a = agg[canon]
        a["usage"] += PW.get(c["prominence"]["type"], 0.5)
    ...
    for canon, a in agg.items():
        if a["on"]:
            continue  # already in Harbor -> not a conversion candidate
        ...
        rows.append((priority, canon, a["axis"], usage, headroom, len(a["labs"]), a["docs"], a["status"]))

    rows.sort(reverse=True)
    print(f"\n{'PRIOR':>6} {'AXIS':10} {'BENCHMARK':24} {'usage':>6} {'headrm':>6} {'labs':>4} {'docs':>4}  status")
    print("-" * 82)
    for pr, canon, axis, usage, hr, labs, docs, status in rows[:30]:
        print(f"{pr:6.2f} {axis:10} {canon:24} {usage:6.1f} {hr:6.2f} {labs:4d} {docs:4d}  {status}")
